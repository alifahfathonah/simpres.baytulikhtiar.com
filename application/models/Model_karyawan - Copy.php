...
class Model_karyawan extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT DISTINCT a.karyawan_id, a.nik, a.fullname, a.foto_karyawan, a.no_ktp, a.tmp_lahir, a.tgl_lahir, a.jk, a.from_pernikahan, a.alamat, 
  ...
  function get_libur_array()
  {
	...
	public	function get_laporan($branch_code, $awal = NULL, $akhir = NULL)
      ...
      if($branch_code == 'semua'){
        ...
        (
          SELECT COUNT(nik) 
          FROM app_absensi_manual  a
          WHERE nik = a.nik AND tanggal BETWEEN CAST(
            (
              SELECT from_date FROM app_cutoff
        ...
        (SELECT COUNT(nik) FROM app_absensi_manual WHERE nik = a.nik AND tanggal BETWEEN CAST((SELECT from_date FROM app_cutoff) AS character varying) 
    ...
    {
      $query = $this->db->query("SELECT a.nik, b.fullname, a.tanggal, a.masuk, a.keluar, a.keterangan FROM app_absensi_manual AS a JOIN app_karyawan AS b ON a.nik = b.nik WHERE a.tanggal BETWEEN '".$from."' AND '".$to."' AND a.nik = '$nik' ORDER BY a.tanggal");
  ...
  function get_absensi_manual_cutoff($from, $thru)
  {
	...
	function get_tempo_kontrak_by_branch($branch, $from_date, $thru_date)
	{
		$query = $this->db->query("SELECT b.nik, a.fullname, d.description AS status, c.from_date AS from_periode, c.thru_date AS thru_periode,
									...
									WHERE b.thru_branch = '$branch' AND c.thru_date BETWEEN '$from_date' AND '$thru_date'");
		return $query->result();
	}
	...
	{
		$query = $this->db->query("SELECT a.nik, a.fullname, b.thru_status, c.description AS status, b.from_date, b.thru_date FROM app_karyawan AS a 
									LEFT JOIN app_mutasi_status AS b ON a.nik = b.nik
									LEFT JOIN app_parameter AS c ON b.thru_status::int = c.parameter_id AND c.parameter_group = 'status' WHERE a.nik = '$nik'
  ...
  {
    $query = $this->db->query("SELECT count(*) num FROM app_absensi_manual WHERE nik = '".$nik."' AND masuk = '' AND keluar = '' AND keterangan = '' AND tanggal BETWEEN '".$from."' AND '".$to."' ");
    $row = $query->row_array();
    return $row['num'];
  }
  ...
  function get_count_ck_date($nik, $from, $to)
  {
	...
	function check_existing_absensi($nik,$from_date){
		$sql = "SELECT COUNT(*) AS jumlah FROM app_absensi_manual WHERE nik = ? and periode_from_date = ?";

		$param = array($nik,$from_date);
